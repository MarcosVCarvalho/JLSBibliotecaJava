...
public class DevolverLivroController {
    ...
    private void confirmarDevolucao() {
        ...
        if (emprestimoSelecionado == null) {
            BibliotecaController.mostrarAlertar( "Nenhum Empréstimo Selecionado", "Selecione um empréstimo para devolver.");
            return;
        }
        ...
        if (sucesso) {
            BibliotecaController.mostrarSucesso( "Devolução Confirmada", "Livro devolvido com sucesso!");
            carregarEmprestimos(); // Atualiza a tabela
        } else {
            BibliotecaController.mostrarAlertar("Erro", "Não foi possível devolver o livro.");
        }
    }
